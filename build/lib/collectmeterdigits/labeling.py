...
def label(path, startlabel=0, imageurlsfile=None):
    ...
    ax2.set_xticklabels(np.arange(0, 10, step=0.2).round(2))
    ...
    #plt.text(-1.0, 0.7, "Prediction:", fontsize=8)
    #predbox = plt.text(-1.0, 0.6, "{:.1f}".format(prediction), fontsize=24, bbox=dict(facecolor='none', edgecolor='black', boxstyle='round,pad=1'))
    ax=plt.gca()
    #ax.get_xaxis().set_visible(False) 
    axlabel =  plt.axes([0.22, 0.05, 0.58, 0.03])
...
def load_image(files, i, startlabel = -1):
    ...
    filename = files[i]
    test_image = Image.open(filename).resize((32, 32))
